...
#  
#
# hw_4_data.csv를 train, validation, test 로 구분한 데이터를 사용하여 리뷰글 긍부정(긍정:1, 부정:0) 예측하는 RNN 모델을 만드시기 바랍니다.
# 
...
#
# 이를 파일('hw_7_RNN_predict_label_본인이름.csv')로 저장하여 보내주시기 바랍니다.
#
# 
#
# 9/2 까지 1.작성한 코드, 2.'hw_7_RNN_predict_label_본인이름.csv' 파일 , 3.과제 발표할 PPT 를 메일로 보내주시기 바랍니다.
# 
# =============================================================================
...
class hw_RNN_model():
    ...
    def create_RNN_model(self):
        ...
        cell = tf.contrib.rnn.MultiRNNCell([tf.contrib.rnn.BasicLSTMCell(128, activation=tf.nn.tanh) for _ in range(4)])
        rnn_output, state = tf.nn.dynamic_rnn(cell, x_data, dtype=tf.float32)
        rnn_output_last = tf.transpose(rnn_output, [1, 0, 2])[-1] 
        
        fc_w_1 = tf.Variable(tf.random_normal([128, 64]))
        fc_b_1 = tf.Variable(tf.zeros(shape=[64]))
        fc_u_1 = tf.add(tf.matmul(rnn_output_last, fc_w_1), fc_b_1)
        fc_z_1 = tf.nn.relu(fc_u_1)
       
        fc_w_2 = tf.Variable(tf.random_normal([64, 2], stddev=0.01))
    ...
    def model_train(self, x_train, y_train, x_vali, y_vali):
        x_data, y_data, loss, acc, pred_y, train = self.create_RNN_model()
...
# =============================================================================    
    tf_model_path = r'C:\Users\CHOYEONGKYU\Desktop\프학 과제\7주차'
    tf_model_name = 'hw_7_RNN_model_CYK'
    
    pred_hw_test_file_path = r'C:\Users\CHOYEONGKYU\Desktop\프학 과제\7주차'
    pred_hw_test_file_name = r'hw_7_RNN_predict_label_CYK.csv'
# =============================================================================   
    
    hw_RNN = hw_RNN_model(tf_model_path, tf_model_name, pred_hw_test_file_path, pred_hw_test_file_name)

    _, x_train, y_train = hw_RNN.load_data_fn(data_file_path, train_file_name, w2v_model)
    _, x_vali, y_vali = hw_RNN.load_data_fn(data_file_path, validation_file_name, w2v_model)
    _, x_test, y_test = hw_RNN.load_data_fn(data_file_path, test_file_name, w2v_model)
    raw_hw_test, x_hw_test, _ = hw_RNN.load_data_fn(hw_test_file_path, hw_test_file_name, w2v_model)
    ...
    hw_RNN.model_train(x_train, y_train, x_vali, y_vali)
    hw_RNN.model_test(x_test, y_test)
    hw_RNN.predict_label(raw_hw_test, x_hw_test)
